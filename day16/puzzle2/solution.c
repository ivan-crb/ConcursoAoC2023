...
int main(int argc, char **argv) {
    ...
    point max = {0, 0, RIGHT};
    int maxEnergizedTiles = findEnergizedTiles(matrix, rowLength, rowsAmount, 0, 0, RIGHT);

    for (int i = 0; i < rowsAmount; i++) {
        for (int j = 0; j < rowLength; j++) {
            if (i == 0) {
                int energizedTiles = findEnergizedTiles(matrix, rowLength, rowsAmount, j, i, DOWN);
            ...
            if (i == rowsAmount - 1) {
                int energizedTiles = findEnergizedTiles(matrix, rowLength, rowsAmount, j, i, UP);
                if (energizedTiles > maxEnergizedTiles) {
                    maxEnergizedTiles = energizedTiles;
                    max.x = j;
                    max.y = i;
            ...
            if (j == 0) {
                int energizedTiles = findEnergizedTiles(matrix, rowLength, rowsAmount, j, i, RIGHT);
                if (energizedTiles > maxEnergizedTiles) {
                    maxEnergizedTiles = energizedTiles;
                    max.x = j;
                    max.y = i;
            ...
            if (j == rowLength - 1) {
                int energizedTiles = findEnergizedTiles(matrix, rowLength, rowsAmount, j, i, LEFT);
                if (energizedTiles > maxEnergizedTiles) {
                    maxEnergizedTiles = energizedTiles;
                    max.x = j;
                    max.y = i;
