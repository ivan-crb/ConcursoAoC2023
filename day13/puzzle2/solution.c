...
int findReflectionHorz (matrix matrix, int exclude) {
    int firstRow = -1;
    int secondRow = -1;
...
int findReflectionVert (matrix matrix, int exclude) {
    int firstCol = -1;
    int secondCol = -1;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < patternsAmount; i++) {
        ...
        int prevRow = findReflectionHorz(patterns[i], -1);
        int prevCol = findReflectionVert(patterns[i], -1);
        for (int j = 0; j < patterns[i].rows && row < 0 && col < 0; j++) {
            for (int k = 0; k < patterns[i].columns && row < 0 && col < 0; k++) {
                if (patterns[i].matrix[j][k] == '#') patterns[i].matrix[j][k] = '.';
                else patterns[i].matrix[j][k] = '#';
            
                row = findReflectionHorz(patterns[i], prevRow);
                col = findReflectionVert(patterns[i], prevCol);
